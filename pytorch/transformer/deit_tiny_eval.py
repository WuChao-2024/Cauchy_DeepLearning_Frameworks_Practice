# DeiT-Tiny 精度评测，计算 TOP-1 和 TOP-5
# 用法: python deit_tiny_eval.py --dataset cifar100 --weights ./outputs/.../deit_tiny_best.pth
...
def evaluate_topk(model, loader, device):
    ...
    for imgs, labels in loader:
        ...
        logits = model(imgs)  # eval 模式返回 (cls+dist)/2
        if device.type == "cuda":
...
def main():
    ...
    print(f"\n{'='*50}")
    print(f"  DeiT-Tiny 框架评测（PyTorch）")
    print(f"{'='*50}")
