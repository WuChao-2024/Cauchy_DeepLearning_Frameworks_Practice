# ResNet-18 训练入口，支持单卡和 DDP 多卡训练，TensorBoard 日志
# 单卡:  python resnet18_train.py --dataset cifar100
# 多卡:  torchrun --nproc_per_node=2 resnet18_train.py --dataset cifar100 --ddp
...
def main():
    ...
    # ── DDP 初始化 ──────────────────────────────────────────────
    # 单卡直接运行；多卡由 torchrun 自动设置 LOCAL_RANK 环境变量
    if args.ddp:
    ...
    # ── 训练产物目录（格式：resnet18_torch_{dataset}_{dtype}_{时间戳}）──
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_name  = f"resnet18_torch_{args.dataset}_{args.dtype}_{timestamp}"
    ...
    # ── 模型 ────────────────────────────────────────────────────
    model = ResNet18(num_classes=num_classes, input_size=input_size).to(device)

    if args.ddp:
        # DDP 包装后，梯度自动在所有 GPU 间 all-reduce 同步
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[local_rank])

    if is_main:
        print_model_summary(model.module if args.ddp else model,
                            (1,) + input_size, title="ResNet-18")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if is_main:
            ...
            if epoch % args.save_freq == 0:
                ckpt = os.path.join(run_dir, f"resnet18_epoch{epoch:03d}.pth")
                torch.save((model.module if args.ddp else model).state_dict(), ckpt)
                print(f"  -> 保存权重: {ckpt}")

            if val_acc > best_acc:
                best_acc = val_acc
                best_path = os.path.join(run_dir, "resnet18_best.pth")
