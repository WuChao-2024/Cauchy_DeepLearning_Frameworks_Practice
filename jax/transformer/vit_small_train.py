# JAX/Flax ViT-Small 训练入口
# 单卡: python vit_small_train.py --dataset cifar100
# 多卡: python vit_small_train.py --dataset cifar100 --pmap
...
# ViT 没有 batch_stats，直接用标准 TrainState
...
def train_step(state, images, labels, use_bf16=False):
    def loss_fn(params):
        ...
        logits = state.apply_fn({'params': params}, images_cast, training=True)
        ...
        loss = -jnp.sum(smooth_labels * jax.nn.log_softmax(logits), axis=-1).mean()
        return loss, logits
...
def main():
    ...
    run_name  = f"vit_small_jax_{args.dataset}_{args.dtype}_{timestamp}"
    ...
    print_jax_model_summary(state.params, title="ViT-Small (JAX)")
