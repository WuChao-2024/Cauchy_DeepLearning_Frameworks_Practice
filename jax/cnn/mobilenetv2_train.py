# JAX/Flax MobileNetV2 训练入口
# 单卡: python mobilenetv2_train.py --dataset cifar100
# 多卡: python mobilenetv2_train.py --dataset cifar100 --pmap
# bf16: python mobilenetv2_train.py --dataset cifar100 --dtype bfloat16
...
def create_train_state(model, rng, input_size, lr, num_steps):
    ...
    tx = optax.chain(
        optax.add_decayed_weights(4e-5),
...
def main():
    ...
    run_name  = f"mobilenetv2_jax_{args.dataset}_{args.dtype}_{timestamp}"
    ...
    print_jax_model_summary(state.params, title="MobileNetV2 (JAX)")
