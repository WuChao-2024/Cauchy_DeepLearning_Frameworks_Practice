# JAX/Flax ResNet-18 精度评测，计算 TOP-1 和 TOP-5
# 用法: python resnet18_eval.py --dataset cifar100 --ckpt_dir ./outputs/.../checkpoints/best
...
def parse_args():
    ...
    p.add_argument("--ckpt_dir",    required=True, help="orbax checkpoint 目录路径")
...
def main():
    ...
    for images, labels in val_iter_fn():
        ...
        logits = eval_step(state, images_jax, jnp.array(labels))
        logits.block_until_ready()  # 等待 XLA 异步计算完成
